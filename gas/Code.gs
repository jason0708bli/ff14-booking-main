...
function doPost(e) {
  try {
    ...
    if (action === 'deleteShift') return deleteShift(data.shiftId);
...
function deleteShift(id) {
  if (!id) {
    return createJsonResponse({ success: false, message: '缺少班次 ID' });
  }

  var bookingSheet = getSheetSafe('併桌團員明細');
  var bookings = bookingSheet.getDataRange().getValues();
  var bookingRows = [];
  for (var j = bookings.length - 1; j >= 1; j--) {
    if (bookings[j][1] === id) {
      bookingRows.push(j + 1);
    }
  }
  bookingRows.sort(function (a, b) { return b - a; });
  for (var k = 0; k < bookingRows.length; k++) {
    bookingSheet.deleteRow(bookingRows[k]);
  }
  ...
  for (var i = 1; i < shifts.length; i++) {
    if (shifts[i][0] === id) {
      ...
      return createJsonResponse({ success: true, message: '班次已刪除' });
    }
  }
  return createJsonResponse({ success: false, message: '找不到該班次' });
}
